...
const AnalyticsPage = () => {
  const { stats, feedback, aiSelection, tickets, isLoading } = useAnalyticsData();
  const ticketRows = tickets.data?.data.tickets ?? [];
  const totalTickets = tickets.data?.data.total ?? stats.data?.totalTickets ?? 0;
  const openTickets = ticketRows.filter((ticket) => ticket.status === "open" || ticket.status === "pending").length || stats.data?.openTickets || 0;
  const resolvedTickets = ticketRows.filter((ticket) => ticket.status === "resolved" || ticket.status === "closed").length || stats.data?.resolvedTickets || 0;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="max-w-7xl mx-auto w-full space-y-6 pb-12">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Tickets" value={totalTickets} icon={Layers} tone="info" />
          <StatCard title="Open Tickets" value={openTickets} icon={AlertCircle} tone="warning" />
          <StatCard title="Resolved Tickets" value={resolvedTickets} icon={CheckCircle} tone="success" />
