...
const DashboardPage = () => {
  ...
  const ticketRows = tickets.data?.data.tickets ?? [];
  const liveTicketStats = {
    totalTickets: tickets.data?.data.total ?? businessStats.data?.totalTickets ?? 0,
    openTickets: ticketRows.filter((ticket) => ticket.status === "open" || ticket.status === "pending").length || businessStats.data?.openTickets || 0,
    resolvedTickets: ticketRows.filter((ticket) => ticket.status === "resolved" || ticket.status === "closed").length || businessStats.data?.resolvedTickets || 0,
  };
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Metric title={isSuperadmin ? "Total Businesses" : "Total Tickets"} value={formatNumber(isSuperadmin ? platformStats.data?.totalBusinesses : liveTicketStats.totalTickets)} icon={isSuperadmin ? Building2 : Layers} tone="info" />
            <Metric title={isSuperadmin ? "Active Businesses" : "Open Tickets"} value={formatNumber(isSuperadmin ? platformStats.data?.activeBusinesses : liveTicketStats.openTickets)} icon={CheckCircle2} tone="success" />
            <Metric title={isSuperadmin ? "Total Users" : "Resolved Tickets"} value={formatNumber(isSuperadmin ? platformStats.data?.totalUsers : liveTicketStats.resolvedTickets)} icon={Users} tone="warning" />
          </div>
