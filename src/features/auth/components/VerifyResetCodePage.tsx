...
export function VerifyResetCodePage() {
  ...
  const email = searchParams.get("email") || (location.state as { email?: string } | null)?.email || "your email";
  const initialUserId = searchParams.get("userId") || (location.state as { userId?: string } | null)?.userId || "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const code = otp.join("");
    if (code.length !== 6 || !userId.trim()) {
      return; 
    }
    await verifyCode({ userId: userId.trim(), code }).catch(() => undefined);
  };
          ...
          <form onSubmit={handleSubmit} className="w-full space-y-8 z-10">
            {!initialUserId && (
              <div className="text-left space-y-2">
                <label htmlFor="reset-user-id" className="text-sm font-medium text-[var(--text-secondary)]">
                  Reset user ID
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-11 w-full rounded-lg border border-[var(--border-default)] bg-[var(--bg-elevated)] pl-10 pr-3 text-sm text-[var(--text-primary)] outline-none transition-all focus:border-[var(--accent-primary)] focus:ring-1 focus:ring-[var(--accent-primary)]"
                    placeholder="Paste user id from reset email"
                    required
                  />
                </div>
              </div>
            ...
            <div className="flex flex-col items-center gap-4">
              <Button
                ...
                disabled={otp.join("").length !== 6 || !userId.trim()}
              >
                Continue
              </Button>
